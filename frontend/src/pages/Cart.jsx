...
export default function Cart() {
    ...
    const checkoutHandler = async (sum) => {
        ...
        dispatch(getAPIkey())
            ...
            .then((key) => {
                dispatch(initiatingCheckout(sum))
                    .then((orderId) => {
    ...
    return (
        <>
            <div className="container max-h-96 ">
                ...
                {isLoading ? (
                    <div>im loading this.....</div>
                ) : (
                    <div className="sm:grid sm:grid-cols-5 sm:grid-rows-4 gap-4 min-h-96 flex-shrink-0">
                        ...
                        <div className="sm:row-start-2 sm:row-span-3 border-t-2 border-orange-700 sm:col-start-4 sm:col-span-2 px-2 rounded-xl shadow-lg  w-full h-60 flex flex-col gap-4">
                            ...
                            <button
                                ...
                                className="py-2 px-4 rounded-lg bg-orange-800 text-white font-semibold active:bg-orange-200 hover:text-black "
                            >
                                Proceed To Checkout
                            </button>
                        </div>
